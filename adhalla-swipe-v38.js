...
function finishScope(){
  ...
  /* V3.8 — email payload is deliberately human-readable.
     Internal scoring metadata stays in the browser and is not emailed. */
  const readableAnswers = scopeAnswers
    .map((answer, index) => `${index + 1}. ${answer.category} — ${answer.label}`)
    .join("\n");

  let readableConclusion = `${title}\n${summary}`;

  if(tensionText){
    readableConclusion += `\n\nMärkus: ${tensionText}`;
  }

  document.getElementById("scope-readable-answers-field").value = readableAnswers;
  document.getElementById("scope-readable-profile-field").value = readableConclusion;
