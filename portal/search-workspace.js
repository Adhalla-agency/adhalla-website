...
export const searchLabels={'search.campaign_class':'Kampaania liik','search.subject':'Lühinimi','search.country':'Riigikood','search.conversion_goal':'Mõõdetav eesmärk','search.start_date':'Alguskuupäev','search.risk_profile':'Pakkumise valdkond','search.separate_opt_in':'Eraldi kampaania nõusolek',...Object.fromEntries(Object.entries(decisions).map(([k,v])=>['confirm.'+k,v+' ülevaatus']))};
...
function field(parent,label,value,{choices,kind='text',max}={}){const wrap=n('label',label),el=n(choices?'select':kind==='textarea'?'textarea':'input');if(choices)for(const [key,text]of choices){const o=n('option',text);o.value=key;el.append(o);}else if(kind!=='textarea')el.type=kind;el.setAttribute('aria-label',label);if(max)el.maxLength=max;el.value=value??'';wrap.append(el);parent.append(wrap);return el;}
export function mountSearch(parent,changed){
 const box=n('section');box.className='wide campaign-essentials';box.append(n('h3','Kampaania ülesehitus'),n('p','Tärniga väljad on enne esitamist kohustuslikud. Eelvaadet saad koostada ka varem.'));
 const body=n('div');body.className='brief-grid';box.append(body);parent.prepend(box);let saved=emptySearch(),separate=false;
 const inputs={campaign_class:field(body,'Kampaania liik *','',{choices:[['','Vali…'],['Service','Teenus'],['Product','Toode'],['Brand','Bränd'],['Competitor','Konkurendid']]}),
 ...
 risk_profile:field(body,'Pakkumise valdkond *','unknown',{choices:[['unknown','Vali…'],['standard','Tavaline toode või teenus'],['finance','Finantsteenus'],['credit','Krediit'],['health','Tervis'],['gambling','Hasartmäng'],['political','Poliitika'],['other_restricted','Muu piiratud valdkond']]})};
 const note=n('p');note.className='wide';body.append(note);
 const update=()=>{const special=['Brand','Competitor'].includes(inputs.campaign_class.value);note.textContent=special?'Eraldi kampaania: sinu allpool valitud päevaeelarve kehtib sellele kampaaniale.'+(!separate?' Enne esitamist peab põhikampaania olema loodud.':''):'';};
 for(const[k,el]of Object.entries(inputs)){el.id='search-'+k;if(k!=='end_date')el.setAttribute('aria-required','true');el.onchange=()=>{update();changed();};}
 return {fill(brief,{separateAvailable=false}={}){saved=structuredClone(brief.search||emptySearch());separate=separateAvailable;for(const[k,el]of Object.entries(inputs))el.value=saved[k]??'';update();},
 read(){return {schema_version:2,search:{...saved,...Object.fromEntries(Object.entries(inputs).map(([k,el])=>[k,el.value||null])),separate_opt_in:['Brand','Competitor'].includes(inputs.campaign_class.value)}};},invalidate(){}};
}
