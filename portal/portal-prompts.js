...
let epoch=0,workflow=null,timer=null;
onAuthStateChanged(getAuth(initializeApp(firebaseConfig)),user=>{
 const capture=++epoch;workflow?.destroy();workflow=null;clearInterval(timer);api.start(user);if(!user)return;
 ...
 async function connect(){if(capture!==epoch||workflow||reading||document.hidden)return;reading=true;
  try{const value=await api.read(base+'/weekly');if(capture!==epoch||!value.client_id)return;
