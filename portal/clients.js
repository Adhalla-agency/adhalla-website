import {connectionSupport} from './connection-support.js?v=0.33';
...
async function open(id){const capture=epoch,pick=++selection;busy=false;$('content').replaceChildren();$('detailStatus').textContent='Laadin ülevaatust…';if(!$('detail').open)$('detail').showModal();
 ...
 if(state==='promoted'){const clientId=review.lifecycle.client_id;const link=node('a','Ava selle kliendi äri ülevaade');link.href='business.html?client='+encodeURIComponent(clientId)+'&release=0.27.1';root.append(link);await sourceForm(root,clientId,capture,pick);return;}
