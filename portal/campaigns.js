...
function review(r){selected=r;$('approvalControls').hidden=false;$('approveCheck').checked=false;$('approve').disabled=true;$('approvalStatus').textContent=states[r.status]||r.status;const dialog=$('requestDialog');let detail=$('requestReview');if(!detail){detail=node('div');detail.id='requestReview';dialog.insertBefore(detail,$('approvalControls'));}if(!dialog.open)dialog.showModal();detail.replaceChildren(node('h3',campaignLabel(r.campaign_id||'campaign-01')+' · klient '+r.client_id+' · versioon '+(r.submission_revision||1)),node('p',r.snapshot.operation==='review_existing_campaign_change'?'Soov: analüüsida olemasoleva kampaania muudatust. Reklaamide muutmine vajab eraldi kontrolli.':'Soov: koostada ja valideerida Search kampaania ning luua see peatatud olekus. Käivitamine ei ole lubatud.'));
