...
import {createCampaignClient,fields,lines,states,help,normalizeBrief,cpcGuidance,approvalPresentation,campaignPresentation,researchPresentation,quotaPresentation} from './campaigns-client.js?v=0.12';
...
function renderCreationReview(detail,r){
 ...
 const approve=node('button','Kinnita peatatud loomine');approve.type='button';approve.disabled=true;check.onchange=()=>approve.disabled=!check.checked;
 approve.onclick=async()=>{if(!check.checked)return;approve.disabled=true;try{await client.write('/worker/requests/'+r.request_id+'/approve_creation',{review_digest:review.review_digest});closeReview();say('Loomine kinnitatud. Adhalla kontrollib Google Adsi seadeid ja loob kampaania peatatud olekus.');await refresh();}catch(e){say(e.message);approve.disabled=!check.checked;}};
