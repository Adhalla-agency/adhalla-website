...
let epoch=0,timer=null,current=null,submitting=false,selectedSource='google_ads',readEpoch=0,questions=null;
...
function render(data){current=data;$('clientIdentity').textContent=(data.name||'Ettevõte')+' · '+data.client_id;$('clientKind').textContent=data.client_id==='0000'?'KAITSTUD SISEKLIENT':'ETTEVÕTTE ÜLEVAADE';const job=data.job,pending=job&&['queued','running'].includes(job.status);$('loadMetrics').disabled=submitting;
 ...
 for(const [key,title,fields] of [['google_ads','Google Ads',[['impressions','Näitamised'],['clicks','Klikid'],['cost','Reklaamikulu',true],['conversions','Konversioonid'],['conversion_value','Konversiooniväärtus',true]]],['ga4','Google Analytics',[['activeUsers','Aktiivsed kasutajad'],['sessions','Seansid'],['engagedSessions','Kaasatud seansid'],['keyEvents','Võtmesündmused'],['totalRevenue','Mõõdetud tulu',true]]]]){
  const source=report.sources[key],section=node('section','', 'card');section.dataset.source=key;section.hidden=key!==selectedSource;section.append(node('h2',title));root.append(section);
 ...
 const chain=node('section');chain.dataset.source='gtm';chain.hidden=selectedSource!=='gtm';root.append(chain);measurementChain(chain,report);
 const gtm=node('section','','card'),inspection=report.gtm;gtm.dataset.source='gtm';gtm.hidden=selectedSource!=='gtm';gtm.append(node('h2','Google Tag Manager'));root.append(gtm);
...
for(const button of document.querySelectorAll('[data-source]'))button.onclick=()=>{selectedSource=button.dataset.source;for(const b of document.querySelectorAll('#sourceTabs button[data-source]'))b.setAttribute('aria-selected',String(b===button));for(const section of document.querySelectorAll('#metrics [data-source]'))section.hidden=section.dataset.source!==selectedSource;};
