...
export function weeklyView(root,api){
 ...
 function render(value){data=value;const report=value.report;
  ...
  if(!report){const status=value.job?.status;root.append(n('p',status==='running'?'AI koostab kokkuvõtet…':status==='limited'?'Kokkuvõte ootab mudeli kasutuslimiidi vabanemist.':status==='failed'?(value.job.attempts>=2?'Selle nädala kaks katset ei õnnestunud. Vajab Adhalla kontrolli.':'Kokkuvõtte koostamine ei õnnestunud. Üks korduskatse toimub automaatselt tunni pärast.'):'Esimest nädalakokkuvõtet pole veel.','source-note'));return;}
  shown=report.run_id;
