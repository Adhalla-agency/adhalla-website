...
export function clientReview(root,state,api,route,clientId,refresh,{dirty=false,beforeDecision=async()=>state,onSubmitted=()=>{},onMissing=()=>{}}={}){
 root.replaceChildren();if(!state.proposal||state.proposal.brief_version!==state.brief?.version)return;
 const exact=state.content_review?.status!=='superseded'&&state.content_review?.proposal_version===state.proposal.version&&state.content_review?.brief_version===state.brief?.version;if(exact){root.append(n('p','✓ See reklaamisisu on juba Adhallale saadetud. Uue soovi jaoks muuda ja salvesta sisu.'));return;}
 const missing=(state.brief?.missing_information||[]).filter(k=>!k.startsWith('confirm.'));if(missing.length){const warning=n('p','Enne esitamist täida puuduvad kampaania väljad.'),go=n('button','Näita puuduvaid välju');go.type='button';go.onclick=onMissing;root.append(warning,go);}
