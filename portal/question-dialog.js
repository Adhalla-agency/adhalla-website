...
export function questionsWorkflow(root,api,base,clientId,{auto=true}={}){
 ...
 weekly.onclick=()=>{if(!latest?.report||busy)return;index=0;seen();renderWeekly();};
 ...
 api.read(base+'/weekly').then(value=>{if(!alive)return;latest=value;const report=value.report;if(!report?.questions?.length)return;weekly.hidden=false;values=Object.fromEntries(report.questions.map(q=>[q.id,value.answers?.answers?.find(a=>a.id===q.id)?.answer||'']));let already=false;try{already=localStorage.getItem(key())==='seen';}catch{}
  const today=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Tallinn'}).format(new Date());const age=(Date.parse(today)-Date.parse(report.period.end))/86400000;
  if(auto&&!already&&!value.answers&&age>=1&&age<=7&&!document.querySelector('dialog[open]')){seen();renderWeekly();}
 }).catch(()=>{/* Optional questions never block the page. */});
 return {destroy(){alive=false;close();root.replaceChildren();}};
}
